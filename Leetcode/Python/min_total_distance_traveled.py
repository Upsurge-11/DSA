...
def main():
  ...
  factory = [list(map(int, input().split())) for _ in range(int(input()))]
...
def minimumTotalDistance(robot: list[int], factory: list[list[int]]) -> int:
  ...
  @lru_cache(None)
  def func(robot_idx: int, factory_idx: int) -> int:
    if robot_idx == len(robot):
      return 0
    if factory_idx == len(factory_positions):
      return float('inf')
    min_distance = func(robot_idx, factory_idx + 1)
    total_distance = 0
    for i in range(factory_limits[factory_idx]):
      if robot_idx + i < len(robot):
        ...
        min_distance = min(min_distance, total_distance + func(robot_idx + i + 1, factory_idx + 1))
      else:
        break
    return min_distance
  return func(0, 0)
